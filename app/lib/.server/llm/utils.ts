...
export function simplifyAiforgeActions(input: string): string {
  // Using regex to match aiforgeAction tags that have type="file"
  const regex = /(<aiforgeAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/aiforgeAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<aiforgeAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</aiforgeAction>`;
    });

  return `<aiforgeArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</aiforgeArtifact>`;
}
