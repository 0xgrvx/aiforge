...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyAiforgeActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyAiforgeActions(content);

      content = content.replace(/<div class=\\"__aiforgeThought__\\">.*?<\/div>/s, '');
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`Aiforge failed to select files`);
  }
