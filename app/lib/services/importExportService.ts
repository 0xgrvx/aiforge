...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          aiforge_user_profile: this._safeGetItem('aiforge_user_profile'),
          aiforge_settings: this._safeGetItem('aiforge_settings'),
          aiforge_profile: this._safeGetItem('aiforge_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('aiforge_viewed_features'),
          developer_mode: this._safeGetItem('aiforge_developer_mode'),
        ...
        ui: {
          // Tab configuration
          aiforge_tab_configuration: this._safeGetItem('aiforge_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('aiforge_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('aiforge_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          aiforge_read_logs: this._safeGetItem('aiforge_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('aiforge_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('aiforge_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.aiforge_tab_configuration) {
        try {
          this._safeSetItem('aiforge_tab_configuration', data.ui.aiforge_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'aiforge_acknowledged_debug_issues',
        'aiforge_acknowledged_connection_issue',
        'error_logs',
        'aiforge_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('aiforge_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
